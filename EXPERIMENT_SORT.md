...
1. **Load the VibeRate skill** before anything else, and follow its brain +
   capture conventions from the start.
...
- **Shot target is *your app's* URL, never VibeRate's** — `push`/`watch` already
  send to VibeRate.
- If `vbrt watch` is running, artifacts and commits **stream live**; otherwise
  they ride the next `vbrt push --all`. Either is fine — pick one and say which.
