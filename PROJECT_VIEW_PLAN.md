...
- [x] **🐛 Context meter is inconsistent — make it stable + always-on (2026-06-23,
  ...
  - **It bounces around a lot mid-session.** The pill is updated from *every*
    ...
    is cache-independent, so cache hits don't explain the jitter. The leading suspect is
    **sub-agent (`Task`) calls**: a spawned sub-agent runs with its *own* small context,
    and if its `message_start` usage reaches the stream it drops the meter to the
    sub-agent's size, then it jumps back to the main thread's. *To verify:* whether
    Claude Code surfaces sub-agent/sidechain `message_start` events in the top-level
  ...
  - **It changes across resume — "doesn't make sense" (Mike's most-important point).**
    ...
    (stale/hidden until the next live turn) and, because the window is chosen from
    `state.drive.model` (`driveCtxWindow`, 200k vs 1M) which is only set from the
    `system` init event (`app.js` L4648), an unknown model **defaults to 200k** and can
    render a wildly wrong % until the next turn re-emits init. Then the *value* itself
    legitimately differs because the ended figure came from the `result` event (full
  ...
  - **Likely auto-compaction, and we don't tell the user.** Claude Code auto-compacts a
    ...
    drops every other `system` subtype (incl. compaction boundaries) into the generic
